...
Before action, read root AGENTS.md, docs/00_START_HERE.md, all Foundation documents in authority order, docs/DECISIONS/2026-08-11-living-memory-doctrine.md, docs/PRODUCT/LIVING_MEMORY_DEFINITION.md, docs/PRODUCT/STORYTELLER_SOVEREIGNTY.md, docs/PRODUCT/LIVING_MEMORY_SHARE_POLICY_V2.md, docs/PRODUCT/LIVING_MEMORY_ANTI_DRIFT_CHECKLIST.md, the current implementation receipts, docs/EXECUTION/README.md, docs/SECURITY/SECRETS_AND_CONFIGURATION.md, and docs/OPERATIONS/ENVIRONMENTS_AND_HANDOFF.md. When TASK_QUEUE selects the one-week proof, also read docs/EXECUTION/ONE_WEEK_LIVING_MEMORY_PROOF_ROADMAP.md and docs/EXECUTION/ONE_WEEK_LIVING_MEMORY_TASK_GRAPH.json.

Preserve validated implementation. Existing MemoryStory, MediaAsset, legacy truth-state fields, receipt, local-first, durability, and identity architecture are compatibility infrastructure beneath the Living Memory aggregate. Do not rename databases or recreate working systems for terminology purity. Legacy truth-state storage must not be surfaced as a judgment of whether a personal recollection is true, false, reliable, or disputed.
...
Muse listens first and exists to help the individual remember. AI may transcribe, organize, retrieve, connect, translate, ask, and present source-grounded material. It must never fact-check, judge, score, correct, or rank a person's recollection; never silently invent testimony; never collapse different recollections into one official version; never replace an original source; and never expose material outside the authorized archive scope.
